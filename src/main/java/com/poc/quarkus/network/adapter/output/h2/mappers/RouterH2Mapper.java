package com.poc.quarkus.network.adapter.output.h2.mappers;



import com.poc.quarkus.network.adapter.output.h2.data.*;
import com.poc.quarkus.network.domain.entity.Router;
import com.poc.quarkus.network.domain.entity.Switch;
import com.poc.quarkus.network.domain.vo.*;
...
public class RouterH2Mapper {
    ...
    public static Router toDomain(RouterData routerData){
        var routerType = com.poc.quarkus.network.domain.vo.RouterType.valueOf(routerData.getRouterType().name());

        var routerId = com.poc.quarkus.network.domain.vo.RouterId.withId(routerData.getRouterId().toString());
        var switchId = com.poc.quarkus.network.domain.vo.SwitchId.withId(routerData.getNetworkSwitch().getSwitchId().toString());
